...
LOCAL_TAURUS_PATH="$HOME/Crypto/vultisig/tss-lib-upgrade/cggmp21/Taurus/Taurus_fork/multi-party-sig/pkg/protocol"


for ip in "${!droplets[@]}"; do
    echo "Updating content on ${droplets[$ip]} ($ip)"
    gnome-terminal -- scp -r $LOCAL_TAURUS_PATH root@$ip:~/Taurus/Taurus_fork/multi-party-sig/pkg/protocol
done

echo "Content updated on all droplets."